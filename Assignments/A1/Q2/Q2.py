...
def EDA():
    ...
    eda_data = pd.read_csv("Train_B_Bayesian.csv")
    ...
    plt.savefig('catplot.jpg',bbox_inches='tight', dpi=300)
...
def five_fold_cross_validation(df_train,laplace_factor):
    ...
    for i in range(5):
        ...
        print(f"Accuracy on fold {i+1}(as test data) is: {pred}")
        mean_accuracy+=pred
...
df_train = remove_outliers(df_train=df_train)
